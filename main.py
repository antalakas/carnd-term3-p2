...
def train_nn(sess, epochs, batch_size, get_batches_fn, train_op, cross_entropy_loss, input_image,
    ...
    """
    # TODO: Implement function
    for epoch_element in range(epochs):
        s_time = time.time()
        for image, label in get_batches_fn(batch_size):
            # Training
            _, loss = sess.run([train_op, cross_entropy_loss],
                               feed_dict={input_image: image, correct_label: label, keep_prob: keep_probability_value,
                                          learning_rate: learning_rate_value})

        print("Epoch: {}".format(epoch_element + 1), "/ {}".format(num_of_epochs), " Loss: {:.3f}".format(loss), " Time: ",
...
def run():
    ...
    with tf.Session() as sess:
        ...
        w1, keep_probability, layer3, layer4, layer7 = load_vgg(sess, vgg_path)
        ...
        train_nn(sess, num_of_epochs, batch_size, get_batches_fn, train_op, cross_entropy_loss, w1, correct_label,
                 keep_probability, learning_rate)
        # TODO: Save inference data using helper.save_inference_samples
        helper.save_inference_samples(runs_dir, data_dir, sess, image_shape, logits, keep_probability, w1)
